...
}`,{label:"class",detail:"definition",type:"keyword"}),o('import {${names}} from "${module}"\n${}',{label:"import",detail:"named",type:"keyword"}),o('import ${name} from "${module}"\n${}',{label:"import",detail:"default",type:"keyword"})],W=new H,j=new Set(["Script","Block","FunctionExpression","FunctionDeclaration","ArrowFunction","MethodDeclaration","ForStatement"]);function Y(O){return(Q,e)=>{let $=Q.node.getChild("VariableDefinition");return $&&e($,O),!0}}const bO=["FunctionDeclaration"],dO={FunctionDeclaration:Y("function"),ClassDeclaration:Y("class"),ClassExpression:()=>!0,EnumDeclaration:Y("constant"),TypeAliasDeclaration:Y("type"),NamespaceDeclaration:Y("namespace"),VariableDefinition(O,Q){O.matchContext(bO)||Q(O,"variable")},TypeDefinition(O,Q){Q(O,"type")},__proto__:null};function k(O,Q){let e=W.get(Q);if(e)return e;let $=[],a=!0;function Z(r,S){let n=O.sliceString(r.from,r.to);$.push({label:n,type:S})}return Q.cursor(K.IncludeAnonymous).iterate(r=>{if(a)a=!1;else if(r.name){let S=dO[r.name];if(S&&S(r,Z)||j.has(r.name))return!1}else if(r.to-r.from>8192){for(let S of k(O,r.node))$.push(S);return!1}}),W.set(Q,$),$}const X=/^[\w$\xa1-\uffff][\w$\d\xa1-\uffff]*$/,w=["TemplateString","String","RegExp","LineComment","BlockComment","VariableDefinition","TypeDefinition","Label","PropertyDefinition","PropertyName","PrivatePropertyDefinition","PrivatePropertyName"];function xO(O){let Q=x(O.state).resolveInner(O.pos,-1);if(w.indexOf(Q.name)>-1)return null;let e=Q.name=="VariableName"||Q.to-Q.from<20&&X.test(O.state.sliceDoc(Q.from,Q.to));if(!e&&!O.explicit)return null;let $=[];for(let a=Q;a;a=a.parent)j.has(a.name)&&($=$.concat(k(O.state.doc,a)));return{options:$,from:e?Q.from:O.pos,validFor:X}}function b(O,Q,e){var $;let a=[];for(;;){let Z=Q.firstChild,r;if(Z?.name=="VariableName")return a.push(O(Z)),{path:a.reverse(),name:e};if(Z?.name=="MemberExpression"&&(($=r=Z.lastChild)===null||$===void 0?void 0:$.name)=="PropertyName")a.push(O(r)),Q=Z;else return null}}function wO(O){let Q=$=>O.state.doc.sliceString($.from,$.to),e=x(O.state).resolveInner(O.pos,-1);return e.name=="PropertyName"?b(Q,e.parent,Q(e)):w.indexOf(e.name)>-1?null:e.name=="VariableName"||e.to-e.from<20&&X.test(Q(e))?{path:[],name:Q(e)}:(e.name=="."||e.name=="?.")&&e.parent.name=="MemberExpression"?b(Q,e.parent,""):e.name=="MemberExpression"?b(Q,e,""):O.explicit?{path:[],name:""}:null}function yO(O,Q){let e=[],$=new Set;for(let a=0;;a++){for(let r of(Object.getOwnPropertyNames||Object.keys)(O)){if($.has(r))continue;$.add(r);let S;try{S=O[r]}catch{continue}e.push({label:r,type:typeof S=="function"?/^[A-Z]/.test(r)?"class":Q?"function":"method":Q?"variable":"property",boost:-a})}let Z=Object.getPrototypeOf(O);if(!Z)return e;O=Z}}function eQ(O){let Q=new Map;return e=>{let $=wO(e);if(!$)return null;let a=O;for(let r of $.path)if(a=a[r],!a)return null;let Z=Q.get(a);return Z||Q.set(a,Z=yO(a,!$.path.length)),{from:e.pos-$.name.length,options:Z,validFor:X}}}const P=z.define({name:"javascript",parser:cO.configure({props:[V.add({IfStatement:c({except:/^\s*({|else\b)/}),TryStatement:c({except:/^\s*({|catch\b|finally\b)/}),LabeledStatement:E,SwitchBody:O=>{let Q=O.textAfter,e=/^\s*\}/.test(Q),$=/^\s*(case|default)\b/.test(Q);return O.baseIndent+(e?0:$?1:2)*O.unit},Block:I({closing:"}"}),ArrowFunction:O=>O.baseIndent+O.unit,"TemplateString BlockComment":()=>null,"Statement Property":c({except:/^{/}),JSXElement(O){let Q=/^\s*<\//.test(O.textAfter);return O.lineIndent(O.node.from)+(Q?0:O.unit)},JSXEscape(O){let Q=/\s*\}/.test(O.textAfter);return O.lineIndent(O.node.from)+(Q?0:O.unit)},"JSXOpenTag JSXSelfClosingTag"(O){return O.column(O.node.from)+O.unit}}),A.add({"Block ClassBody SwitchBody EnumBody ObjectExpression ArrayExpression":J,BlockComment(O){return{from:O.from+2,to:O.to-2}}})]}),languageData:{closeBrackets:{brackets:["(","[","{","'",'"',"`"]},commentTokens:{line:"//",block:{open:"/*",close:"*/"}},indentOnInput:/^\s*(?:case |default:|\{|\}|<\/)$/,wordChars:"$"}}),q={test:O=>/^JSX/.test(O.name),facet:F({commentTokens:{block:{open:"{/*",close:"*/}"}}})},UO=P.configure({dialect:"ts"},"typescript"),fO=P.configure({dialect:"jsx",props:[u.add(O=>O.isTop?[q]:void 0)]}),_O=P.configure({dialect:"jsx ts",props:[u.add(O=>O.isTop?[q]:void 0)]},"typescript"),WO="break case const continue default delete export extends false finally in instanceof let new return static super switch this throw true typeof var yield".split(" ").map(O=>({label:O,type:"keyword"}));function aQ(O={}){let Q=O.jsx?O.typescript?_O:fO:O.typescript?UO:P;return new N(Q,[P.data.of({autocomplete:L(w,D(mO.concat(WO)))}),P.data.of({autocomplete:xO}),O.jsx?uO:[]])}function vO(O){for(;;){if(O.name=="JSXOpenTag"||O.name=="JSXSelfClosingTag"||O.name=="JSXFragmentTag")return O;if(!O.parent)return null;O=O.parent}}function v(O,Q,e=O.length){for(let $=Q?.firstChild;$;$=$.nextSibling)if($.name=="JSXIdentifier"||$.name=="JSXBuiltin"||$.name=="JSXNamespacedName"||$.name=="JSXMemberExpression")return O.sliceString($.from,Math.min($.to,e));return""}const TO=typeof navigator=="object"&&/Android\b/.test(navigator.userAgent),uO=B.inputHandler.of((O,Q,e,$)=>{if((TO?O.composing:O.compositionStarted)||O.state.readOnly||Q!=e||$!=">"&&$!="/"||!P.isActiveAt(O.state,Q,-1))return!1;let{state:a}=O,Z=a.changeByRange(r=>{var S,n;let{head:t}=r,l=x(a).resolveInner(t,-1),g;if(l.name=="JSXStartTag"&&(l=l.parent),$==">"&&l.name=="JSXFragmentTag")return{range:m.cursor(t+1),changes:{from:t,insert:"></>"}};if($=="/"&&l.name=="JSXFragmentTag"){let p=l.parent,s=p?.parent;if(p.from==t-1&&((S=s.lastChild)===null||S===void 0?void 0:S.name)!="JSXEndTag"&&(g=v(a.doc,s?.firstChild,t))){let y=`/${g}>`;return{range:m.cursor(t+y.length),changes:{from:t,insert:y}}}}else if($==">"){let p=vO(l);if(p&&((n=p.lastChild)===null||n===void 0?void 0:n.name)!="JSXEndTag"&&a.sliceDoc(t,t+2)!="</"&&(g=v(a.doc,p,t)))return{range:m.cursor(t+1),changes:{from:t,insert:`></${g}>`}}}return{range:r}});return Z.changes.empty?!1:(O.dispatch(Z,{userEvent:"input.type",scrollIntoView:!0}),!0)});function ZQ(O,Q){return Q||(Q={parserOptions:{ecmaVersion:2019,sourceType:"module"},env:{browser:!0,node:!0,es6:!0,es2015:!0,es2017:!0,es2020:!0},rules:{}},O.getRules().forEach((e,$)=>{e.meta.docs.recommended&&(Q.rules[$]=2)})),e=>{let{state:$}=e,a=[];for(let{from:Z,to:r}of P.findRegions($)){let S=$.doc.lineAt(Z),n={line:S.number-1,col:Z-S.from,pos:Z};for(let t of O.verify($.sliceDoc(Z,r),Q))a.push(hO(t,$.doc,n))}return a}}function T(O,Q,e,$){return e.line(O+$.line).from+Q+(O==1?$.col-1:-1)}function hO(O,Q,e){let $=T(O.line,O.column,Q,e),a={from:$,to:O.endLine!=null&&O.endColumn!=1?T(O.endLine,O.endColumn,Q,e):$,message:O.message,source:O.ruleId?"eslint:"+O.ruleId:"eslint",severity:O.severity==1?"warning":"error"};if(O.fix){let{range:Z,text:r}=O.fix,S=Z[0]+e.pos-$,n=Z[1]+e.pos-$;a.actions=[{name:"fix",apply(t,l){t.dispatch({changes:{from:l+S,to:l+n,insert:r},scrollIntoView:!0})}}]}return a}export{uO as autoCloseTags,wO as completionPath,ZQ as esLint,aQ as javascript,P as javascriptLanguage,fO as jsxLanguage,xO as localCompletionSource,eQ as scopeCompletionSource,mO as snippets,_O as tsxLanguage,UO as typescriptLanguage};
//# sourceMappingURL=index-COqZhKgE.js.map
